          ...
          const mockRecords = [
            {
              ...
              status: 'paid',
              description: `${subscription.plan?.name || 'Professional'} Plan - Monthly Subscription`
            },
            {
              id: 'inv_002',
      ...
      <Card style={styles.card}>
        <Card.Content>
          ...
          <View style={styles.subscriptionDetails}>
            ...
            <Text style={styles.planPrice}>${subscription.plan.price}/{subscription.plan.interval}</Text>
          </View>
          
          <Text style={styles.subscriptionStatus}>
            Status: <Text style={getStatusStyle(subscription.status)}>{getStatusText(subscription.status)}</Text>
          </Text>
      ...
      return (
        <Card style={styles.card}>
          <Card.Content>
            ...
            <Text style={styles.noHistoryText}>No billing history available</Text>
          </Card.Content>
        </Card>
      );
    }
            ...
            <View key={record.id}>
              ...
              >
                <View style={styles.invoiceDetails}>
                  <Text style={styles.invoiceDate}>{formatDate(record.date)}</Text>
                  <Text style={styles.invoiceDescription}>{record.description}</Text>
                  <Text style={getInvoiceStatusStyle(record.status)}>{getInvoiceStatusText(record.status)}</Text>
                </View>
                <View style={styles.invoiceAmount}>
                  <Text style={styles.amountText}>${record.amount.toFixed(2)}</Text>
