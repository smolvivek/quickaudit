/**
 * Script to fix TypeScript errors in hooks
 */
...
// Fix useScreenGestures.ts
const fixUseScreenGestures = () => {
  const dirPath = path.join(process.cwd(), 'src/hooks');
...
export default useScreenGestures;`;
  ...
  fs.writeFileSync(path.join(dirPath, 'useScreenGestures.ts'), content, 'utf8');
  console.log('Fixed useScreenGestures.ts');
};

// Fix useScreenState.ts
const fixUseScreenState = () => {
  const dirPath = path.join(process.cwd(), 'src/hooks');
  ensureDirectoryExists(dirPath);
...
export default useScreenState;`;
  ...
  fs.writeFileSync(path.join(dirPath, 'useScreenState.ts'), content, 'utf8');
  console.log('Fixed useScreenState.ts');
};
...
// Run the functions
console.log('Fixing hooks and payment screen...');
fixUseScreenGestures();
fixUseScreenState();
fixPaymentScreen();
console.log('Hooks and payment screen fixed successfully!');
