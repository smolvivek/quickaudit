...
jest.mock('@stripe/stripe-react-native', () => ({
  CardField: ({ onCardChange, ...props }) => (
    <div
      ...
      onChange={() => onCardChange({ complete: true, brand: 'visa', last4: '4242' })}
      {...props}
    />
  ),
  useStripe: () => ({
  ...
  useConfirmPayment: () => ({
    confirmPayment: jest.fn().mockResolvedValue({ paymentIntent: { id: 'pi_123456789' } }),
    loading: false
  })
}));
...
describe('PaymentMethodScreen', () => {
  ...
  it('shows loading state during submission', () => {
    ...
    require('@stripe/stripe-react-native').useStripe = () => ({
      createPaymentMethod: jest.fn().mockImplementation(() => new Promise(resolve => setTimeout(resolve, 1000))),
      loading: true
    });
